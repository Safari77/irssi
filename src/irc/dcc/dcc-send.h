...
#define DCC_SEND(dcc) MODULE_CHECK_CAST_MODULE(dcc, SEND_DCC_REC, type, "DCC", "SEND")

#define IS_DCC_SEND(dcc) (DCC_SEND(dcc) ? TRUE : FALSE)
...
#include <irssi/src/irc/dcc/dcc-file-rec.h>
	...
	/* fastsending: */
	unsigned int waitforend
	    : 1; /* file is sent, just wait for the replies from the other side */
	unsigned int gotalldata : 1; /* got all acks from the other end (needed to make sure the end
	                                of transfer works right) */
} SEND_DCC_REC;
